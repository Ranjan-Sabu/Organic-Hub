...
def user_register(request): 
  ...
  if request.method == 'POST':
    ...
    if form.is_valid():
       ...
       message = render_to_string('user_template/verification_email.html',{
          'user': user,
          'domain': current_site,
    ...
    if request.method == 'POST':
        ...
        if user is not None:  # Check if the user is active
            try:
                ...
                print(f'is_cart_item_exists: {is_cart_item_exists}')  # Add this line for debugging
...
def forgotPassword(request):
   if request.method =='POST':
      ...
      if Registration.objects.filter(email=email).exists():
         ...
         message = render_to_string('user_template/reset_password_email.html',{
          'user': user,
          'domain': current_site,
          'uid': urlsafe_base64_encode(force_bytes(user.pk)),
          'token': default_token_generator.make_token(user),
